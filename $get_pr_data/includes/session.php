...
$get_all_emp1_sql = "SELECT * FROM employeedetail WHERE Status = 'Active'  ORDER BY EmpLname";
...
$get_sched_sql = "SELECT * FROM createjo INNER JOIN payroll INNER JOIN schedule WHERE `createjo`.`objid` = `payroll`.`id_no` AND `schedule`.`JobOrderNo` = `payroll`.`JobOrderNo` AND `schedule`.`no`= 1 ORDER BY `payroll`.`JobOrderNo` DESC ";
$get_sched_data = $con->prepare($get_sched_sql);
$get_sched_data->execute();

$get_sss_sql = "SELECT * FROM employeedetail INNER JOIN sss INNER JOIN no WHERE `sss`.`EmpCode` = `employeedetail`.`EmpCode` AND `sss`.`Empcode` = `no`.`EmpCode`";
