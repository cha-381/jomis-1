<?php
  ...
  if ($_FILES['pdfFile']['type'] == "application/pdf" or $_FILES['pdfFile']['type'] == "application/jpg") {
    ...
    $dest_files = "../dist/recom".$_FILES['pdfFile']['name'];
    ...
    if (file_exists($dest_files)) {
      ...
      $alert_msg = "File location : ../dist/recom".$_FILES['pdfFile']['name']."<br/>";
      }
    }
  }
  else {
...
$register_user_sql = "INSERT INTO recommendation SET 
        ID            = :ID,
        Filenames     = :pdfFile
        ";

$sql ="UPDATE `employeedetail` SET Filenames   = '$dest_files'
        WHERE   ID               = '$joID' ";
