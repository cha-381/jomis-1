...
$get_o_sql= "SELECT COUNT(`code`) as total FROM `createjo` ";
...
$get_emm_sql= "SELECT COUNT(`JobOrderNo`) as total FROM `schedule` where `JobOrderNo`='$JobOrder' ";
$get_emm_data = $con->prepare($get_emm_sql);
$get_emm_data->execute();
$get_emm_data->setFetchMode(PDO::FETCH_ASSOC);
while ($result1 = $get_emm_data->fetch(PDO::FETCH_ASSOC)) {
  $count_emm =  $result1['total'];
}

$get_em2_sql= "SELECT SUM(`TotalAmount`) as total, id FROM `schedule` where `JobOrderNo`='$JobOrder' ";
$get_em2_data = $con->prepare($get_em2_sql);
$get_em2_data->execute();
$get_em2_data->setFetchMode(PDO::FETCH_ASSOC);
while ($result1 = $get_em2_data->fetch(PDO::FETCH_ASSOC)) {
  $count_em2 =  $result1['total'];
  $count_id =  $result1['id'];
}
